...
class _GuestHomePageState extends State<GuestHomePage> {
  ...
  final List<_FacilityItem> _roomTypes = const [
    ...
    _FacilityItem(title: 'Spa', icon: Icons.spa_outlined),
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _FacilityCard(
            title: item.title,
            icon: item.icon,
            onTap: () => debugPrint('Facility tapped: ${item.title}'),
          );
        },
      ),
    );
...
class _FacilityItem {
  final String title;
  final IconData icon;
  const _FacilityItem({required this.title, required this.icon});
}
